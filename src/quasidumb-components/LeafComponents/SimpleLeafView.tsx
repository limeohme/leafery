...
import { deleteLeaf, ILeaf, updateLeafProp } from '../../services/leaf-service';
...
export default function SimpleLeafView ({ leaf, setTrigger, trigger }: SimpleLeafProps) {
  const { context: { theme, user } } = useContext(AppState);
  ...
  return (
    <Paper sx={{ 
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-evenly' }}>
        {leaf.pinned? <RemoveCircleIcon aria-label='remove from pinned leaves' sx={{ cursor: 'pointer', '&:hover': { color: lightTheme.accent } }}
          onClick={() => {
            updateLeafProp(user, leaf.id, 'pinned', false);
            setTrigger(!trigger);
          }}/>:
          <PushPinIcon aria-label='add to pinned leaves' sx={{ cursor: 'pointer', '&:hover': { color: lightTheme.accent } }}
            onClick={() => {
              updateLeafProp(user, leaf.id, 'pinned', true);
              setTrigger(!trigger);
            }}
          ></PushPinIcon>}
        <LaunchIcon aria-label='edit leaf' sx={{ cursor: 'pointer', '&:hover': { color: lightTheme.accent } }}
